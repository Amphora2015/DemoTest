if object_id('dbo.usp_get_real_port_nums', 'P') is null
   exec('create procedure dbo.usp_get_real_port_nums as select getdate()')
go

  
ALTER procedure dbo.usp_get_real_port_nums  
(  
...
select port_num   
   ...
   select @port_num = min(port_num)  
   from #port_num_pool p  
   where exists (select 1  
                 from dbo.trade_item ti  
                 where p.port_num = ti.real_port_num)  
  
   while @port_num is not null  
   begin  
      if @debugonoff = 'Y'  
      begin  
         select @smsg = 'DEBUG(3): Processing the port_num #' + convert(varchar, @port_num) + ' ...'  
         print ' '  
         print @smsg  
      end  
      ...
      if exists (select 1  
                 ...
                 where ti.real_port_num = @port_num and  
  ...
  if @debugonoff = 'Y' 
   ...
   begin  
      print ' '  
      print 'DEBUG: ' + LTRIM(RTRIM(Str(@rows_returned))) + ' rows returned.'  
   end  
...
go
grant exec on dbo.usp_get_real_port_nums to next_usr
go

IF OBJECT_ID('dbo.usp_get_real_port_nums') IS NOT NULL
   PRINT '<<< CREATED/UPDATED PROCEDURE dbo.usp_get_real_port_nums >>>'
ELSE
   PRINT '<<< FAILED TO CREATE/UPDATE PROCEDURE dbo.usp_get_real_port_nums >>>'
go
