if object_id('dbo.fetchDeletedAllocItemsForAlloc','P') is null
 exec ('create procedure dbo.fetchDeletedAllocItemsForAlloc as select getdate()')
go  

alter procedure dbo.fetchDeletedAllocItemsForAlloc 
(       
   @alloc_num       int 
) 
as     
...
endofsp: 
--drop table #deleted_ai 
if @errcode > 0
   return 1
return 0
GO

IF OBJECT_ID('dbo.fetchDeletedAllocItemsForAlloc') IS NOT NULL
   PRINT '<<< CREATED PROCEDURE dbo.fetchDeletedAllocItemsForAlloc >>>'
ELSE
   PRINT '<<< FAILED CREATING PROCEDURE dbo.fetchDeletedAllocItemsForAlloc >>>'
GO
